"""R5-P2 preregistered progressive-return paired latency pilot."""
...
from rlccl.transport.reference_full_moe import RETURN_META_FIELDS, seed_reference_experts  # noqa: E402
...
def _job_inputs(seed: int, family: str, job: int, rank: int, *, smoke: bool) -> dict[str, Any]:
    ...
    if seed not in allowed:
        raise ValueError("seed not allowed by R5-P2 preregistration")
...
def _pair_order(seed_index: int, family_index: int, job: int) -> tuple[str, str]:
    return ARMS if (seed_index + family_index + job) % 2 == 0 else tuple(reversed(ARMS))


def _semantic_returns(result: dict[str, Any]) -> list[dict[str, Any]]:
    return _descriptor_equivalence(
        result["return_descriptors"],
        ("descriptor_index", "sendcounts_tokens", "offsets_tokens", "tokens", "metadata_digest", "output_digest"),
    )


def _compare(arms: dict[str, dict[str, Any]], outputs: dict[str, np.ndarray]) -> dict[str, Any]:
    p2, e0 = arms["P2"], arms["E0"]
    return_tokens_p2 = sum(int(value["tokens"]) for value in p2["return_descriptors"])
    return_tokens_e0 = sum(int(value["tokens"]) for value in e0["return_descriptors"])
    bytes_per_token = RETURN_META_FIELDS * np.dtype(np.int64).itemsize + EXPERT_OUTPUT * np.dtype(np.float32).itemsize
    checks = {
        "same_router_topk": p2["topk_digests"] == e0["topk_digests"],
        "same_router_assignments": p2["router_assignment_digest"] == e0["router_assignment_digest"],
        "same_forward_descriptors": _descriptor_equivalence(
            p2["forward_descriptors"],
            ("chunk_ids", "sendcounts_tokens", "offsets_tokens", "tokens", "metadata_digest", "feature_digest"),
        ) == _descriptor_equivalence(
            e0["forward_descriptors"],
            ("chunk_ids", "sendcounts_tokens", "offsets_tokens", "tokens", "metadata_digest", "feature_digest"),
        ),
        "same_expert_batches_shapes_count_weights_outputs": p2["expert"] == e0["expert"],
        "same_return_descriptors": _semantic_returns(p2) == _semantic_returns(e0),
        "same_return_descriptor_count": len(p2["return_descriptors"]) == len(e0["return_descriptors"]),
        "same_return_tokens": return_tokens_p2 == return_tokens_e0,
        "same_return_bytes": return_tokens_p2 * bytes_per_token == return_tokens_e0 * bytes_per_token,
        "same_scheduler_actions": p2["scheduler_actions"] == e0["scheduler_actions"],
        "all_arm_correctness": all(
            arms[name]["correctness"]["final_combine_correct"]
            and arms[name]["correctness"]["token_integrity"]
            for name in ARMS
        ),
        "no_future_return_access": all(
            arms[name]["correctness"]["return_future_access"] == 0 for name in ARMS
        ),
    }
    max_abs = float(np.max(np.abs(outputs["P2"] - outputs["E0"])))
    checks["final_outputs_equivalent"] = bool(
        np.allclose(outputs["P2"], outputs["E0"], atol=2e-3, rtol=2e-3)
    )
    return {
        **checks, "max_abs_output_difference": max_abs,
        "return_tokens": return_tokens_p2,
        "return_payload_bytes": return_tokens_p2 * bytes_per_token,
...
def main() -> None:
    ...
    if world_size != 2:
        raise RuntimeError("R5-P2 requires exactly two NCCL ranks")
        ...
        for seed_index, seed in enumerate(seeds):
            for family in families:
                ...
                for job in range(jobs_per_family):
                    ...
                    order = _pair_order(seed_index, family_index, job)
                    for arm in order:
                        dist.barrier()
                        arms[arm] = _run_arm(
                            arm=arm, case_name=f"r5-p2-{seed}-{family}-{job}", rank=rank,
                            ...
                            progressive_expert=False, progressive_return=arm == "P2",
                    ...
                    equivalence = _compare(arms, outputs)
                    if not equivalence["pass"]:
                        raise RuntimeError(f"R5-P2 arm equivalence failed: {equivalence}")
                    local_pairs.append({
                        "seed": seed, "family": family, "job": job,
                        "arm_order": list(order), "arms": arms, "equivalence": equivalence,
                    })
                    dist.barrier()
    finally:
        bridge.stop()
    ...
    if rank == 0:
        ...
        for pair_index in range(len(local_pairs)):
            ...
            row: dict[str, Any] = {
                key: exemplar[key] for key in ("seed", "family", "job", "arm_order")
            }
            row["rank_equivalence"] = {
                str(rank_id): rank_pairs[rank_id]["equivalence"]
            ...
            row["delta_return_us"] = (
                row["E0"]["primary_makespan_us"] - row["P2"]["primary_makespan_us"]
            )
            row["pass"] = all(value["pass"] for value in row["rank_equivalence"].values())
            rows.append(row)
        if not all(row["pass"] for row in rows):
            raise RuntimeError("R5-P2 aggregate equivalence failed")
        result = {
            "schema_version": 1,
            "study": "R5-P2 progressive return pilot",
            ...
            "frozen_protocol": {
                ...
                "jobs_per_family": jobs_per_family, "pairs": len(rows),
                "smoke": args.allow_smoke, "arms": list(ARMS),
                "full_size_expert_batches": True,
                "same_full_batch_instrumentation_both_arms": True,
                "return_descriptor_split_merge_reorder": False,
                "return_progressive_only_in_P2": True,
                "combine_progressive": False, "profiler": False,
                "partial_shards_ratio": 0.75, "checkpoint8": True,
            },
            "pairs": rows, "pass": True,
        }
        output = args.output_dir / (
            "r5_p2_smoke_host.json" if args.allow_smoke else "r5_p2_primary_host.json"
        )
        output.write_text(json.dumps(result, indent=2, sort_keys=True) + "\n", encoding="utf-8")
        print(json.dumps({
            "output": str(output), "sha256": sha256_file(output),
            "pairs": len(rows), "pass": True,
        }, indent=2))
    dist.barrier()
