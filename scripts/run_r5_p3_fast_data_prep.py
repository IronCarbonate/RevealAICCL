"""R5-P3 preregistered fast progressive data-preparation pilot."""
...
from rlccl.transport.reference_full_moe import FORWARD_META_FIELDS, seed_reference_experts  # noqa: E402
...
def _job_inputs(seed: int, family: str, job: int, rank: int, *, smoke: bool) -> dict[str, Any]:
    ...
    if seed not in allowed:
        raise ValueError("seed not allowed by R5-P3 preregistration")
...
def _arm_order(seed_index: int, family_index: int, job: int) -> tuple[str, ...]:
    index = seed_index * len(FAMILIES) * JOBS_PER_FAMILY + family_index * JOBS_PER_FAMILY + job
    return ARM_ORDERS[index % len(ARM_ORDERS)]


def _semantic_forward(result: dict[str, Any]) -> list[dict[str, Any]]:
...
def _semantic_return(result: dict[str, Any]) -> list[dict[str, Any]]:
    return _descriptor_equivalence(
...
def _compare(arms: dict[str, dict[str, Any]], outputs: dict[str, np.ndarray]) -> dict[str, Any]:
    reference = arms["E0"]
    checks = {
        "same_router_topk": all(arms[name]["topk_digests"] == reference["topk_digests"] for name in ARMS),
        "same_router_assignments": all(
            arms[name]["router_assignment_digest"] == reference["router_assignment_digest"] for name in ARMS
        ),
        "byte_exact_forward_descriptors": all(
            _semantic_forward(arms[name]) == _semantic_forward(reference) for name in ARMS
        ),
        "same_scheduler_actions": all(
            arms[name]["scheduler_actions"] == reference["scheduler_actions"] for name in ARMS
        ),
        "same_expert_batches_weights_outputs": all(
            arms[name]["expert"] == reference["expert"] for name in ARMS
        ),
        "same_return_descriptors": all(
            _semantic_return(arms[name]) == _semantic_return(reference) for name in ARMS
        ),
        "all_arm_correctness": all(
            arms[name]["correctness"]["final_combine_correct"]
            and arms[name]["correctness"]["token_integrity"] for name in ARMS
        ),
    }
    max_abs = {
        "E1_vs_E0": float(np.max(np.abs(outputs["E1"] - outputs["E0"]))),
        "D1_vs_E0": float(np.max(np.abs(outputs["D1"] - outputs["E0"]))),
        "D1_vs_E1": float(np.max(np.abs(outputs["D1"] - outputs["E1"]))),
    }
    checks["final_outputs_equivalent"] = all(
        np.allclose(outputs[left], outputs[right], atol=2e-3, rtol=2e-3)
        for left, right in (("E1", "E0"), ("D1", "E0"), ("D1", "E1"))
    )
...
def main() -> None:
    ...
    if world_size != 2:
        raise RuntimeError("R5-P3 requires exactly two NCCL ranks")
        ...
        for seed_index, seed in enumerate(seeds):
            for family in families:
                ...
                for job in range(jobs_per_family):
                    ...
                    order = _arm_order(seed_index, family_index, job)
                    for arm in order:
                        dist.barrier()
                        fast = arm in ("E1", "D1")
                        arms[arm] = _run_arm(
                            arm=arm, case_name=f"r5-p3-{seed}-{family}-{job}", rank=rank,
                            ...
                            fast_data_prep=fast, overlap_count_with_h2d=fast,
                    ...
                    if not equivalence["pass"]:
                        raise RuntimeError(f"R5-P3 arm equivalence failed: {equivalence}")
                    local_pairs.append({
    ...
    if rank == 0:
        ...
        for pair_index in range(len(local_pairs)):
            ...
            row["ranks"] = {
                str(rank_row["rank"]): rank_pairs[rank_row["rank"]]["arms"] for rank_row in gathered
            }
            for arm in ARMS:
                ...
                precompute = max(
                    ...
                    for value in rank_arms
                )
                row[arm] = {
                    "primary_makespan_us": (done - first) / 1e3,
                    "static_precompute_us": precompute,
                    "precompute_inclusive_makespan_us": (done - first) / 1e3 + precompute,
                }
            row["delta_fast_us"] = row["E0"]["primary_makespan_us"] - row["E1"]["primary_makespan_us"]
            row["delta_progressive_us"] = row["D1"]["primary_makespan_us"] - row["E1"]["primary_makespan_us"]
            row["pass"] = all(value["pass"] for value in row["rank_equivalence"].values())
            rows.append(row)
        if not all(row["pass"] for row in rows):
            raise RuntimeError("R5-P3 aggregate equivalence failed")
        result = {
            "schema_version": 1, "study": "R5-P3 fast progressive data-preparation pilot",
            ...
            "frozen_protocol": {
                ...
                "jobs_per_family": jobs_per_family, "pairs": len(rows), "smoke": args.allow_smoke,
                "arms": list(ARMS), "fast_arms": ["E1", "D1"],
                "descriptor_groups": [[0], [1], [2], [3], [4], [5], [6, 7]],
                "count_exchange_prestarted_during_packing_only_in_E1": True,
                "count_h2d_overlap_fast_arms": True,
                "expert_progressive": False, "return_progressive": False,
        ...
        output = args.output_dir / (
            "r5_p3_smoke_host.json" if args.allow_smoke else "r5_p3_primary_host.json"
        )
