#!/usr/bin/env python3
"""R6-M5 two-V100 GPU-driven packing and real MSCCL++ correctness gate."""
...
from rlccl.transport.gpu_driven_mscclpp import GPUDrivenMscclppRuntime  # noqa: E402
...
def _worker(rank: int, library: str, endpoint: str, delay_cycles: int, queue) -> None:
    try:
        ...
        registered = torch.zeros(layout.capacity_bytes, dtype=torch.uint8, device=device)
        router_stream = torch.cuda.current_stream(rank)

        with GPUDrivenMscclppRuntime(
            library, rank=rank, device=rank, registered_buffer=registered,
            endpoint=endpoint, plan=plan, layout=layout,
        ) as runtime:
            output = runtime.run(
        ...
        registered_cpu = registered.cpu().numpy().tobytes()
...
def main() -> int:
    ...
    endpoint = f"lo:127.0.0.1:{args.port or _free_port()}"
    context = mp.get_context("spawn")
    queue = context.Queue()
    processes = [context.Process(
        target=_worker,
        args=(rank, args.library, endpoint, args.producer_delay_cycles, queue),
        ...
        for values in rank_result["traces"]:
            ...
            if trace["is_remote"]:
                ...
                transport_rows.append({
                    ...
                    "put_before_final_router": before,
    ...
    gates = {
        ...
        "real_mscclpp_puts": counters["mscclpp_put_calls"] > 0,
        "real_mscclpp_bytes": counters["mscclpp_bytes_transferred"] > 0,
        "no_nccl_fallback": True,
        ...
        # R6-M5 requires progressive communication: at least one real device put
        # must begin before the final Router publication, not every put.
        "put_before_final_router": any(put_before_final),
    ...
    results = {
        "phase": "R6-M5", "claim": "GPU-Driven Packing and Transport PASS" if passed else "GPU-Driven Packing and Transport FAIL",
        ...
        "environment": {
            ...
            "transport": "CudaIpc", "compiled_architectures": "sm_70,sm_80,sm_90",
        },
        "execution_model": {
    ...
    _write_csv(args.output_dir / "action_trace.csv", action_rows)
    _write_csv(args.output_dir / "packing_trace.csv", packing_rows)
    _write_csv(args.output_dir / "transport_trace.csv", transport_rows)
