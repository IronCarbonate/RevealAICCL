"""R4-F0 formal validation on the preregistered fresh corpus."""
...
from scripts.run_r4_a0_c0_full_moe import (  # noqa: E402
    EXPERT_HIDDEN, EXPERT_OUTPUT, EXPERT_SEED, ROUTER_SEED, _run_arm,
)
from scripts.run_r4_p0_full_moe import _equivalent  # noqa: E402


FORMAL_SEEDS = (9042, 9142, 9242)
FORMAL_JOBS_PER_FAMILY = 20
SMOKE_SEED = 8042


def _job_inputs(seed: int, family: str, job: int, rank: int, *, smoke: bool) -> dict[str, Any]:
    allowed = (SMOKE_SEED,) if smoke else FORMAL_SEEDS
    if seed not in allowed:
        raise ValueError("seed is not allowed by the R4-F0 protocol")
...
def main() -> None:
    ...
    parser.add_argument("--jobs-per-family", type=int)
    args = parser.parse_args()
    if args.allow_smoke:
        seeds = tuple(args.seed or (SMOKE_SEED,))
        families = tuple(args.family or (FAMILIES[0],))
        jobs_per_family = int(args.jobs_per_family or 1)
        if seeds != (SMOKE_SEED,):
            raise ValueError("smoke mode may only use the already-consumed P0 seed 8042")
    else:
        seeds, families, jobs_per_family = FORMAL_SEEDS, FAMILIES, FORMAL_JOBS_PER_FAMILY
        if args.seed or args.family or args.jobs_per_family is not None:
            raise ValueError("canonical R4-F0 corpus is immutable")

    dist.init_process_group("nccl", init_method="env://")
    rank, world_size = dist.get_rank(), dist.get_world_size()
    if world_size != 2:
        raise RuntimeError("R4-F0 requires exactly two NCCL ranks")
            ...
            for family in families:
                ...
                for job in range(jobs_per_family):
                    ...
                    for arm in _pair_order(seed_index, family_index, job):
                        dist.barrier()
                        arms[arm] = _run_arm(
                            arm=arm, case_name=f"r4-f0-{seed}-{family}-{job}", rank=rank,
    ...
    if rank == 0:
        ...
        for pair_index in range(len(local_pairs)):
            ...
            row["ranks"] = {
                str(rank_row["rank"]): {arm: rank_pairs[rank_row["rank"]][arm] for arm in ("C", "D")}
                for rank_row in gathered
            }
            row["equivalence"] = {
                str(rank_id): _equivalent(rank_pairs[rank_id]["C"], rank_pairs[rank_id]["D"])
        ...
        if not all(row["pass"] for row in rows):
            raise RuntimeError("R4-F0 paired semantic equivalence failed")
        result = {
            "schema_version": 1, "study": "R4-F0 formal full-reference-MoE validation",
            ...
            "frozen_protocol": {"seeds": list(seeds), "families": list(families), "jobs_per_family": jobs_per_family, "pairs": len(rows), "formal": not args.allow_smoke, "profiler": False, "expert_progressive": False, "partial_shards_ratio": 0.75, "checkpoint8": True, "only_variable": "forward descriptor execution timing"},
            "pairs": rows, "pass": True,
        }
        output = args.output_dir / ("r4_f0_smoke_host.json" if args.allow_smoke else "r4_f0_primary_host.json")
