// R6-M6: frozen GPU scheduler/packer -> backend-neutral transport -> real NCCL LSA.
// M4/M5 sources and their action/record semantics are not modified.
...
#include "rlccl/transport/cuda/nccl_gin_transport.cuh"
...
struct M6TransportCounters {
  uint64_t scheduler_actions;
  uint64_t transport_actions;
  uint64_t pack_calls;
  uint64_t lsa_transfers;
  uint64_t lsa_bytes_transferred;
  ...
  while (true) {
    ...
    if (error_code == kTransportOk) {
      if (action.dst_rank != action.src_rank) {
        ...
        __syncthreads();
        uint64_t* destination = reinterpret_cast<uint64_t*>(
            ncclGetPeerPointer(window, physical.physical_dst_offset, action.dst_rank));
        ...
        if (!destination) {
          if (threadIdx.x == 0) error_code = kTransportInvalidRank;
        } else {
        ...
        if (error_code == kTransportOk) {
          ncclLsaBarrierSession<ncclCoopCta> completion{
              ncclCoopCta(), dev_comm, ncclTeamTagLsa(), action.descriptor_id};
          completion.arrive(ncclCoopCta(), cuda::memory_order_release);
        }
        if (threadIdx.x == 0 && error_code == kTransportOk) {
          if (trace) trace->t6_put_end = global_time_ns();
          counters->lsa_transfers += 1;
          counters->lsa_bytes_transferred += physical.physical_bytes;
...
__device__ void remote_wait_progress_role(
  ...
  for (uint32_t index = 0; index < expected_remote_actions; ++index) {
    ncclLsaBarrierSession<ncclCoopCta> completion{
        ncclCoopCta(), dev_comm, ncclTeamTagLsa(), index};
...
__global__ void gpu_transport_progress_kernel(
  ...
  gpu_transport_progress_role(
      dev_comm, window, blob, source_rank, action_queue, state, records_by_descriptor,
...
__global__ void gpu_transport_pipeline_kernel(
  ...
  if (blockIdx.x == 0) {
    ...
    remote_wait_progress_role(dev_comm, expected_remote_actions, wait_times, counters);
  }
}
...
struct Runtime {
  ...
  M6TransportCounters* d_counters = nullptr;
  ...
  Runtime(int rank_, int device_, const ncclUniqueId* unique_id, size_t bytes,
          const void* plan_blob, size_t plan_bytes)
      : rank(rank_), remote_rank(rank_ ^ 1), device(device_),
        registered_bytes(bytes) {
    CUDA_CHECK(cudaSetDevice(device));
    if (!unique_id) throw std::runtime_error("M6 NCCL unique ID is null");
    if (plan_bytes < sizeof(CompiledPlanBlobHeader)) throw std::runtime_error("M6 plan header truncated");
    std::memcpy(&plan_header, plan_blob, sizeof(plan_header));
    if (plan_header.magic != kCompiledPlanMagic || plan_header.world_size != 2 ||
        bytes != 2 * plan_header.region_bytes) throw std::runtime_error("M6 plan/buffer layout mismatch");
    NCCL_CHECK(ncclGetVersion(&nccl_version));
    NCCL_CHECK(ncclCommInitRank(&comm, 2, *unique_id, rank));
    ...
    NCCL_CHECK(ncclCommWindowRegister(
        comm, registered_buffer, registered_bytes, &window, NCCL_WIN_COLL_SYMMETRIC));
    if (!window) throw std::runtime_error("NCCL_DEVICE_API_NOT_AVAILABLE: symmetric window is null");
    ncclDevCommRequirements_t requirements = NCCL_DEV_COMM_REQUIREMENTS_INITIALIZER;
    requirements.lsaBarrierCount = plan_header.max_descriptors;
    NCCL_CHECK(ncclDevCommCreate(comm, &requirements, &dev_comm));
    dev_comm_created = true;
    if (dev_comm.lsaSize < 2)
      throw std::runtime_error("NCCL_DEVICE_API_NOT_AVAILABLE: LSA team size < 2");
    ...
    d_counters = allocate_device<M6TransportCounters>();
    ...
    CUDA_CHECK(cudaMemset(d_counters, 0, sizeof(M6TransportCounters)));
  ...
  void run(
    ...
    gpu_transport_pipeline_kernel<<<4, 256, 0, producer_stream>>>(
        ...
        dev_comm, window, d_plan, d_action_queue, d_state, packing, d_slot_claims,
...
const char* r6_m6_last_error() { return last_error.c_str(); }

size_t r6_m6_unique_id_size() { return sizeof(ncclUniqueId); }

int r6_m6_get_unique_id(void* output, size_t bytes) {
  return protect([&] {
...
void* r6_m6_create(int rank, int device, const void* unique_id, size_t unique_id_bytes,
                   size_t bytes, const void* plan_blob, size_t plan_bytes) {
  ...
  if (protect([&] {
        runtime = new Runtime(rank, device, static_cast<const ncclUniqueId*>(unique_id),
                              bytes, plan_blob, plan_bytes);
      }) != 0)
    return nullptr;
  return runtime;
}

int r6_m6_run(void* opaque, const int64_t* reveal_records, uint32_t record_count,
              const int32_t* destination_ranks, const int64_t* expert_ids,
              const int64_t* token_ids, const int64_t* feature_digests,
              const float* features, int64_t* metadata, uint32_t total_assignments,
              uint32_t feature_width, uint32_t expected_remote_actions,
...
uint64_t r6_m6_counter(void* opaque, int index) {
...
uint64_t r6_m6_capability(void* opaque, int index) {
...
size_t r6_m6_action_count(void* opaque) { return static_cast<Runtime*>(opaque)->actions.size(); }
size_t r6_m6_trace_count(void* opaque) { return static_cast<Runtime*>(opaque)->traces.size(); }
size_t r6_m6_timing_count(void* opaque) { return static_cast<Runtime*>(opaque)->timings.size(); }

int r6_m6_copy_actions(void* opaque, int64_t* output, size_t rows) {
  return protect([&] {
...
int r6_m6_copy_traces(void* opaque, uint64_t* output, size_t rows) {
  return protect([&] {
...
int r6_m6_copy_timings(void* opaque, uint64_t* output, size_t rows) {
  return protect([&] {
...
int r6_m6_copy_buffer(void* opaque, void* output, size_t bytes) {
  return protect([&] {
    const auto* runtime = static_cast<Runtime*>(opaque);
    if (!output || bytes != runtime->registered_bytes)
      throw std::runtime_error("registered buffer output size mismatch");
    CUDA_CHECK(cudaMemcpy(
        output, runtime->registered_buffer, bytes, cudaMemcpyDeviceToHost));
  });
}

void r6_m6_destroy(void* opaque) { delete static_cast<Runtime*>(opaque); }
