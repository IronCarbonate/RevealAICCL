"""Paired-arm owner for the R6-M9 GPU event/timeline runtime."""
...
class GPUE2EPerfRuntime:
    """Own the frozen M7 forward path and the M8 full-handle return path."""

    @staticmethod
    def get_unique_id(library: str | Path) -> bytes:
        lib = ctypes.CDLL(str(library))
        lib.r6_m9_unique_id_size.restype = ctypes.c_size_t
        size = int(lib.r6_m9_unique_id_size())
        storage = ctypes.create_string_buffer(size)
        lib.r6_m9_get_unique_id.argtypes = [ctypes.c_void_p, ctypes.c_size_t]
        if lib.r6_m9_get_unique_id(storage, size) != 0:
            lib.r6_m9_last_error.restype = ctypes.c_char_p
            raw = lib.r6_m9_last_error()
    ...
    def __init__(
        ...
        self._lib.r6_m9_last_error.restype = ctypes.c_char_p
        self._lib.r6_m9_create.argtypes = [
        ...
        self._lib.r6_m9_create.restype = ctypes.c_void_p
        self._lib.r6_m9_run.argtypes = [
            ctypes.c_void_p, ctypes.c_void_p, ctypes.c_uint32,
            ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p,
            ctypes.c_void_p, ctypes.c_uint32, ctypes.c_uint32,
            ctypes.c_uint32, ctypes.c_uint32, ctypes.c_uint32,
            ctypes.c_uint32, ctypes.c_uint64, ctypes.c_size_t,
        ]
        self._lib.r6_m9_run.restype = ctypes.c_int
        self._lib.r6_m9_last_e2e_ms.argtypes = [ctypes.c_void_p]
        self._lib.r6_m9_last_e2e_ms.restype = ctypes.c_float
        for name in ("counter", "combine_counter", "capability"):
            function = getattr(self._lib, f"r6_m9_{name}")
            function.argtypes = [ctypes.c_void_p, ctypes.c_int]
            function.restype = ctypes.c_uint64
        for name in (
            "num_recv_tokens", "num_local_experts", "return_trace_count",
            "num_source_tokens", "trace_count", "timing_count",
        ):
            function = getattr(self._lib, f"r6_m9_{name}")
        ...
        for name in (
            ...
            "copy_traces", "copy_timings", "copy_gate_timing",
            "copy_stage_timing",
        ):
            function = getattr(self._lib, f"r6_m9_{name}")
            function.argtypes = [ctypes.c_void_p, ctypes.c_void_p, ctypes.c_size_t]
            function.restype = ctypes.c_int
        self._lib.r6_m9_destroy.argtypes = [ctypes.c_void_p]
        layout = dispatch_layout
        self._handle = self._lib.r6_m9_create(
    ...
    def _raise(self, operation: str) -> None:
        raw = self._lib.r6_m9_last_error()
        detail = raw.decode(errors="replace") if raw else "unknown error"
        raise RuntimeError(f"R6-M9 {operation} failed: {detail}")
    ...
    def _copy(self, name: str, values: np.ndarray) -> None:
        ...
        if getattr(self._lib, f"r6_m9_copy_{name}")(
    ...
    def run(
        ...
        arm: str, producer_delay_cycles: int, router_stream: int,
    ) -> dict[str, Any]:
        ...
        arm_value = {"progressive": 0, "delayed": 1}.get(arm)
        if arm_value is None:
            raise ValueError("arm must be 'progressive' or 'delayed'")
        status = self._lib.r6_m9_run(
            ...
            arm_value, int(producer_delay_cycles), int(router_stream),
        )
        if status != 0:
            self._raise("run")
        num_recv = int(self._lib.r6_m9_num_recv_tokens(self._handle))
        num_local = int(self._lib.r6_m9_num_local_experts(self._handle))
        trace_count = int(self._lib.r6_m9_return_trace_count(self._handle))
        dispatch_trace_count = int(self._lib.r6_m9_trace_count(self._handle))
        timing_count = int(self._lib.r6_m9_timing_count(self._handle))
        arrays = {
            ...
            "traces": np.empty((dispatch_trace_count, 11), dtype=np.uint64),
            "timings": np.empty((timing_count, 8), dtype=np.uint64),
            "gate_timing": np.empty(4, dtype=np.uint64),
            "stage_timing": np.empty(6, dtype=np.uint64),
        ...
        return {
            **arrays,
            "dispatch_counters": {
                name: int(self._lib.r6_m9_counter(self._handle, index))
                for index, name in enumerate(COUNTER_NAMES)
            },
            "combine_counters": {
                name: int(self._lib.r6_m9_combine_counter(self._handle, index))
                for index, name in enumerate(COMBINE_COUNTER_NAMES)
            },
            "capability": {
                name: int(self._lib.r6_m9_capability(self._handle, index))
                for index, name in enumerate(CAPABILITY_NAMES)
            },
            "arm": arm,
            "e2e_ms": float(self._lib.r6_m9_last_e2e_ms(self._handle)),
            "handle": {
    ...
    def close(self) -> None:
        if self._handle:
            self._lib.r6_m9_destroy(self._handle)
            self._handle = None
...
__all__ = ["COMBINE_COUNTER_NAMES", "GPUE2EPerfRuntime"]
